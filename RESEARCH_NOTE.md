...
This is an elementary task-specific deduction from established basis facts. The [literature comparison](docs/LITERATURE_COMPARISON.md) now gives complete reductions through affine-basis sumsets and fundamental-circuit uniqueness, preserving coordinate multiplicities and zero images. No independent geometric-novelty claim is made.
...
The [excess-distortion theorem](docs/EXCESS_DISTORTION.md) gives a stronger operational form. The same rate is achievable deterministically with at most an $\varepsilon$ fraction of wrong pairs on every input. Below the rate, it determines the sharp exponential decay of the best probability of such a low-distortion answer table. Each hypothetical query still receives its own one-read budget; this is not simultaneous correctness or protection against an adversarially selected erroneous pair.
