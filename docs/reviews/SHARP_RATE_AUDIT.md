...
Differentiating $`\ln2\,c(b)`$ gives $`\mathrm{atanh}b`$, so
...
For a fixed strategy, let $`A_t`$ have the pair-labelled residual rows, let $`\alpha_t`$ be the affine error offset, and put $`r=\mathrm{rank}A_t`$. Uniform input induces the uniform law on the affine image, so exactly
