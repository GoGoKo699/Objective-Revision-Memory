...
**Current research stage, 22 September 2026:** the user selected **Information Processing Letters** and specified that **manuscript preparation is the final step**, then requested a furnished research repository. Begin with the [reading guide](READING_GUIDE.md), [status](STATUS.md), and [research roadmap](RESEARCH_ROADMAP.md). The complete proof packet and bounded comparisons are already recorded through PR #8; navigation work is authorized, and does not initiate journal drafting. Assess the remaining narrow contribution rather than repeat the initial audit from scratch. The [contribution guide](../CONTRIBUTING.md) gives the current collaboration workflow.
...
The original Nisan-Rudich-Saks *Products and Help Bits in Decision Trees* text has now been inspected; the [literature ledger](LITERATURE_COMPARISON.md), Sections 5 and 11, records the weaker matching-based converse and its scope. This supersedes the initial retrieval obligation. For further comparisons, prioritize a concrete candidate implication of the decoder-ball theorem identified in the current roadmap. Independently verify bibliographic identities and theorem numbers for every source relied upon. Record access failures; do not treat an abstract, secondhand citation, or unsuccessful search as a full comparison.
