...
The fifth checks the [strategy reduction](OPERATIONAL_REDUCTION.md) at three
bits. It exhausts all 512 strategies built from the eight distinct constant
or signed-coordinate answer functions for each pair. Across every mask and
input it checks 98,304 pair-error identities, 32,768 total-distortion identities,
12,288 address/class checks, and 16,384 uniform-mask hit counts at all four
integer error thresholds. It also verifies the infinite-parity-penalty
counterexample: a zero-bit code succeeds on exactly four even-parity inputs,
