...
$$\mathcal R(\varepsilon)=\frac{a\eta-\ln\cosh a}{\ln2},\qquad
...
\qquad \operatorname{Var}_{Q_n}(W_n)\le\tfrac14\sum_{k=0}^{n-1}k^2=O(n^3).$$
...
\qquad\operatorname{Var}_{Q_n}(L_n)=O_{a'}(n).$$
...
classical deductions. It supports an internal short-note assessment. It does
not by itself establish historical novelty, publication significance, a
general data-structure theorem, or a theory of objective preservation.
